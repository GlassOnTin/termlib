...
class TerminalScreenStateUrlTest {
    ...
    fun singleLineUrlTrimsTrailingPunctuation() {
        ...
        assertEquals("https://example.com/path", state.getHyperlinkUrlAt(0, 10, autoDetectUrls = true))
        // Haven fork: a tap on the trailing punctuation cell still resolves
        // to the (trimmed) URL via the blob fallback, rather than null —
        // users aiming at the end of a URL shouldn't miss by one cell.
        assertEquals("https://example.com/path", state.getHyperlinkUrlAt(0, 28, autoDetectUrls = true))
    }
    ...
    fun unrelatedIndentedTextIsNotContinuation() {
        ...
        // Haven fork: an indented next row starting with a /path IS joined —
        // CLI tools (Claude Code et al.) hard-wrap URLs at their own width
        // and print the continuation behind alignment padding, so this shape
        // is treated as a wrap. Upstream treats it as unrelated prose; the
        // fork accepts the rare false join to keep real wrapped URLs tappable.
        assertEquals("https://example.com/foo/usr/bin/local", state.getHyperlinkUrlAt(0, 15, autoDetectUrls = true))
        assertEquals("https://example.com/foo/usr/bin/local", state.getHyperlinkUrlAt(1, 5, autoDetectUrls = true))
    }
    ...
    fun trailingPunctuationDoesNotForceContinuation() {
        ...
        // Haven fork: tapping the continuation-shaped row itself resolves via
        // the blob fallback (row 0 fills the width and row 1 is a /-bearing
        // run — the shape of a column-boundary wrap), while a tap on the URL
        // row above keeps the conservative trimmed answer.
        assertEquals("https://example.com/sub.dir/file", state.getHyperlinkUrlAt(1, 2, autoDetectUrls = true))
    }
    ...
    /**
     * Haven #385: the bare-domain branch had no trailing boundary, so it
    ...
    fun maxContinuationRowsLimitBreaksScanning() {
        ...
        // Haven fork: the tap-time blob fallback is bounded by the visible
        // screen rather than MAX_URL_CONTINUATION_ROWS, so all seven rows
        // join. (The bulk grid scan still honours the 6-row cap.)
        assertEquals(
